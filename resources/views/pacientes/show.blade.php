    ...
    <div class="py-1">
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            ...
            @if ($w === 'diagnostico_ya_existe' && !empty($ctx['diagnostico_id']))
                <div
                    class="mb-4 border border-yellow-200 bg-yellow-50 text-yellow-900 rounded-lg p-4 flex items-start justify-between gap-4">
                    <div class="text-sm">
                ...
                <div class="p-6 bg-blue-800 text-white flex justify-between items-start">
                    <div>
                        <p class="text-blue-100 mt-1">
                            Nombre: <span class="font-semibold">
                    ...
                    <div class="flex items-start space-x-4 text-lg self-start pt-1">
                        ...
                        @else
                            <a href="{{ route('pacientes.index') }}" class="hover:text-gray-200"
                                ...
                                <div class="space-y-2 text-sm md:text-base">
                                    ...
                                    <p>
                                        ...
                                        @if (!is_null($paciente->imc))
                                            <span
                                                class="inline-flex items-center px-3 py-1 rounded-full text-xs md:text-sm
...
@php
    $organosEvaluados = collect($scoresNihActuales)
        ->filter(fn($score) => !is_null($score) && $score !== '')
        ->count();

    $ultimoDiagnostico = ($paciente->diagnosticos ?? collect())
        ->sortByDesc('fecha_diagnostico')
        ->first();
@endphp

<div class="border border-blue-100 rounded-lg p-5 bg-blue-50 shadow-sm">
    ...
    <div class="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-4">
        <div>
            ...
            <p class="text-sm text-gray-700 mt-1">
                Utiliza los síntomas activos y la evaluación NIH para proponer un diagnóstico clínico.
            </p>
        </div>

        @if($ultimoDiagnostico)
            <div class="bg-white border rounded-lg px-4 py-3 text-sm min-w-[220px]">
                <p class="text-gray-500">Último diagnóstico</p>

                <p class="font-semibold text-gray-800">
                    {{ $ultimoDiagnostico->tipo_enfermedad ?? 'No definido' }}

                    @if($ultimoDiagnostico->grado_eich)
                        — {{ $ultimoDiagnostico->grado_eich }}
                    @endif
                </p>
            </div>
        @endif
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-5">
        ...
        <div class="bg-white border rounded-lg p-4 flex flex-col justify-between min-h-[116px]">
            <div>
                ...
                <p class="text-2xl font-semibold text-gray-900 mt-1">
                    {{ count($sintomasActivosIds) }}
                </p>
            </div>

            <button type="button"
                onclick="openSintomasModal()"
                class="mt-3 w-full bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded-md text-xs font-semibold shadow-sm">
                Registrar síntomas
            </button>
        </div>
        ...
        <div class="bg-white border rounded-lg p-4 flex flex-col justify-between min-h-[116px]">
            ...
            <button type="button"
                onclick="openOrganosModal()"
                class="mt-3 w-full bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-1.5 rounded-md text-xs font-semibold shadow-sm">
                Evaluar órganos NIH
            </button>
        </div>

        <div class="bg-white border rounded-lg p-4 flex flex-col justify-center min-h-[116px]">
            <p class="text-xs uppercase tracking-wide text-gray-500">
                Estado
            </p>

            <p class="text-sm font-semibold mt-2
                {{ $tieneSintomasActivos ? 'text-green-700' : 'text-yellow-700' }}">
                {{ $tieneSintomasActivos ? 'Preparado para inferencia' : 'Pendiente de síntomas activos' }}
            </p>
        </div>
    ...
    <div class="flex justify-center">
        <form method="POST" action="{{ route('diagnosticos.inferir', $paciente->id) }}">
            ...
            <button type="submit"
                @if(!$tieneSintomasActivos) disabled @endif
                class="px-8 py-2.5 rounded-md shadow text-sm font-semibold min-w-[280px]
                @if($tieneSintomasActivos)
                            ...
                            @foreach ($paciente->diagnosticos as $diagnostico)
                                <div id="modal-diagnostico-{{ $diagnostico->id }}"
                                    ...
                                    <div class="bg-white rounded-lg shadow-xl max-w-3xl w-full mx-4 overflow-hidden">
                                        ...
                                        <div
                                            class="px-6 py-4 bg-blue-700 text-white flex justify-between items-center">
                                            <div>
                                        ...
                                        <div class="px-6 py-4 space-y-4 text-sm text-gray-800">
                                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                <div>
                                                    <h4 class="font-semibold text-gray-700 mb-1">Datos del diagnóstico
                                                    </h4>
                                                    <p><strong>Tipo enfermedad:</strong>
                                                        {{ $diagnostico->tipo_enfermedad ?? '-' }}</p>
                                                    <p><strong>Grado EICH:</strong>
                                                        {{ $diagnostico->grado_eich ?? '-' }}</p>
                                                ...
                                                <div>
                                                    <h4 class="font-semibold text-gray-700 mb-1">Regla / Enfermedad
                                                        asociada</h4>
                                                    <p><strong>Enfermedad:</strong>
                                                        {{ optional($diagnostico->enfermedad)->nombre ?? 'No especificada' }}
                                                    </p>
                                                    <p><strong>Regla clínica:</strong>
                                                        {{ optional($diagnostico->regla)->nombre ?? 'ID ' . ($diagnostico->regla_decision_id ?? '-') }}
                                                    </p>
                                            ...
                                            <div>
                                                    ...
                                                    <div class="max-h-60 overflow-y-auto border rounded">
                                                        <table class="min-w-full text-xs">
                                                            <thead class="bg-gray-50">
                                                                <tr>
                                                                    <th
                                                                        class="px-2 py-1 text-left font-semibold text-gray-600">
                                                                        Órgano</th>
                                                                    <th
                                                                        class="px-2 py-1 text-left font-semibold text-gray-600">
                                                                        Síntoma</th>
                                                ...
                                                @else
                                                    <p class="text-gray-600">No hay síntomas asociados a este
                                                        diagnóstico.</p>
                                                @endif
                                            </div>
                                        </div>
                                ...
                                <div class="flex items-center justify-between mb-3">
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <a href="{{ route('pacientes.trasplantes.create', $paciente) }}"
                                            class="{{ $iconBtn }}" title="Registrar trasplante"
                                            aria-label="Registrar trasplante">
                                            <i class="fas fa-plus"></i>
                                        </a>

                                        {{-- Ver listado (si existe tu route trasplantes.index) --}}
                                        <button type="button" onclick="openTrasplantesModal()"
                                            class="{{ $iconBtn }}" title="Ver trasplantes"
                                        ...
                                        @foreach ($trasplantes as $t)
                                            <li class="flex items-center justify-between gap-3">
                                                ...
                                                <a href="{{ route('trasplantes.show', $t) }}"
                                                    class="text-gray-500 hover:text-gray-800" title="Abrir"
                                ...
                                <div class="flex items-center justify-between mb-3">
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        <a href="{{ route('pacientes.pruebas.create', $paciente) }}"
                                            class="{{ $iconBtn }}" title="Registrar prueba"
                                        ...
                                        @if (\Illuminate\Support\Facades\Route::has('pruebas.index'))
                                            <button type="button" onclick="openPruebasModal()"
                                                class="{{ $iconBtn }}" title="Ver pruebas"
    ...
    {{-- MODAL SÍNTOMAS --}}
    <div id="modal-sintomas"
        class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 hidden">
        <div class="bg-white rounded-lg shadow-xl max-w-3xl w-full mx-4 overflow-hidden">

            <div class="px-6 py-4 bg-blue-700 text-white flex justify-between items-center">
                <h3 class="text-lg font-semibold">Registro de síntomas</h3>
                <button type="button" onclick="closeSintomasModal()"
            ...
            <form method="POST" action="{{ route('pacientes.sintomas.store', $paciente) }}">
                ...
                <div class="p-6 max-h-96 overflow-y-auto">
                    @php
                        $sintomasAgrupados = $sintomasCatalogo
                            ->sortBy([
                                fn($a, $b) => strcmp($a->organo->nombre ?? '', $b->organo->nombre ?? ''),
                                fn($a, $b) => strcmp($a->sintoma ?? '', $b->sintoma ?? ''),
                            ])
                            ->groupBy(fn($sintoma) => $sintoma->organo->nombre ?? 'Sin órgano asignado');
                    @endphp
                    ...
                    <div class="p-6 max-h-96 ">
                        @php
                            ...
                            $columnaIzquierda = collect();
                            $columnaDerecha = collect();
                            $index = 0;

                            foreach ($sintomasAgrupados as $organoNombre => $sintomasOrgano) {
                                if ($index % 2 === 0) {
                                    $columnaIzquierda->put($organoNombre, $sintomasOrgano);
                                } else {
                                    $columnaDerecha->put($organoNombre, $sintomasOrgano);
                                }
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-10">
                            @foreach ([$columnaIzquierda, $columnaDerecha] as $columna)
                                <div class="space-y-6">
                                    @foreach ($columna as $organoNombre => $sintomasOrgano)
                                        <div>
                                            <h4
                                                class="text-sm font-semibold text-blue-800 uppercase tracking-wide mb-2 border-b pb-1">
                                                {{ $organoNombre }}
                                            </h4>

                                            <div class="space-y-2">
                                                @foreach ($sintomasOrgano as $sintoma)
                ...
                <div class="p-6 max-h-96 overflow-y-auto space-y-4">
                        ...
                        @foreach ($organosCatalogo as $organo)
                            ...
                            <div class="border rounded p-3 @if (!$tieneSintomas) opacity-60 @endif">
                                ...
                                <select name="organos[{{ $oid }}][score_nih]"
                                    ...
                                    @for ($i = 0; $i <= 4; $i++)
                                        <option value="{{ $i }}"
                                            @if ((string) $valor === (string) $i) selected @endif>
                                            {{ $i }}
                                        </option>
                                    @endfor
                                </select>
                                ...
                                @if (!$tieneSintomas)
                                    ...
                                    <input type="hidden" name="organos[{{ $oid }}][score_nih]"
                                        value="">
                                @endif
                            </div>
                        @endforeach
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="date" name="fecha_evaluacion" value="{{ now()->toDateString() }}"
                                class="mt-1 w-full border rounded px-2 py-1 text-sm">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="comentario" maxlength="255"
                                class="mt-1 w-full border rounded px-2 py-1 text-sm" placeholder="Observaciones...">
                        </div>
                    </div>
                                    ...
                                    <table class="min-w-full text-sm">
                                        <thead class="bg-white">
                                            <tr>
                                                ...
                                                <th class="px-3 py-2 text-left font-semibold text-gray-600">Resultado
                                                </th>
                                            </tr>
                                        </thead>
                                        <tbody class="divide-y divide-gray-100">
                                            @foreach ($items as $p)
                                                <tr>
                                                    <td class="px-3 py-2">{{ $p->fecha?->format('d/m/Y') ?? '-' }}
                                                    </td>
