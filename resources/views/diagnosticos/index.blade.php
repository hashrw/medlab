                    ...
                    <table class="min-w-full bg-white border border-gray-300 rounded-lg">
                        ...
                        <tbody class="text-gray-700 text-sm">
                            @foreach ($diagnosticos as $diagnostico)
                                @php
                                    $ultimoTrasplante = $diagnostico->paciente
                                        ? $diagnostico->paciente->trasplantes()->orderByDesc('fecha_trasplante')->first()
                                        : null;

                                    $fechaTrasplante = $ultimoTrasplante?->fecha_trasplante
                                        ? $ultimoTrasplante->fecha_trasplante->format('d/m/Y')
                                        : '-';
                                @endphp
                                    ...
                                    <td class="py-3 px-6 text-left whitespace-nowrap">
                                        <div class="flex items-center">
                                            <span class="font-medium">
                                                {{ $diagnostico->estado?->nombre ?? __('Sin registro') }}
                                            </span>
                                        </div>
                                    </td>

                                    <td class="py-3 px-4">{{ $fechaTrasplante }}</td>

                                    <td class="py-3 px-4">
                                        {{ $diagnostico->origen?->origen ?? 'Manual' }}
                                    </td>
