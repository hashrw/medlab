            ...
            <div class="bg-white shadow-lg rounded-lg overflow-hidden">
                ...
                @if(Auth::user()->es_sintoma)
                    <div class="p-6 bg-blue-800 text-white flex justify-between items-center">
                        ...
                        <a href="{{ route('pruebas.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded">
                        ...
                        <tbody class="text-gray-700 text-sm">
                            @foreach ($pruebas as $prueba)
                                <tr class="border-b hover:bg-gray-50">
                                    ...
                                    <td class="py-3 px-4">{{ $prueba->nombre }}</td>
                                    <td class="py-3 px-4">{{ $prueba->tipo_prueba }}</td>
                                    <td class="py-3 px-4">{{ $prueba->fecha }}</td>
                                    <td class="py-3 px-4">{{ $prueba->resultado }}</td>
                                    <td class="py-3 px-4">{{ $prueba->comentario }}</td>
                                    <td class="py-3 px-4 text-center">
                                        <div class="flex justify-center space-x-2">
                                            <a href="{{ route('pruebas.show', $prueba->id) }}" class="text-gray-600 hover:text-blue-500">
                                            ...
                                            @if(Auth::user()->es_sintoma)
                                                <a href="{{ route('pruebas.edit', $prueba->id) }}" class="text-gray-600 hover:text-blue-500">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                                                         fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                ...
                                                <form id="delete-form-{{ $prueba->id }}" method="POST" action="{{ route('pruebas.destroy', $prueba->id) }}"
