        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-lg rounded-lg overflow-hidden">
                <div class="p-6 bg-blue-800 text-white flex justify-between items-center">
                    ...
                    <a href="{{ route('enfermedads.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded">
                        + Nuevo Registro
                    </a>
                </div>
                        ...
                        <tbody class="text-gray-700 text-sm">
                            @foreach ($enfermedads as $enfermedad)
                                <tr class="border-b hover:bg-gray-50">
                                    <td class="py-3 px-4">{{ $enfermedad->id }}</td>
                                    <td class="py-3 px-4">{{ $enfermedad->tipo_trasplante }}</td>
                                    <td class="py-3 px-4">{{ $enfermedad->fecha_trasplante->format('d/m/Y') }}</td>
                                    <td class="py-3 px-4">{{ $enfermedad->origen_trasplante }}</td>
                                    <td class="py-3 px-4">{{ $enfermedad->identidad_hla }}</td>
                                    <td class="py-3 px-4">{{ $enfermedad->tipo_acondicionamiento }}</td>
                                    <td class="py-3 px-4">{{ $enfermedad->seropositividad_donante }}</td>
                                    <td class="py-3 px-4">{{ $enfermedad->seropositividad_receptor }}</td>
                                    <td class="py-3 px-4 text-center">
                                        <div class="flex justify-center space-x-2">
                                            <a href="{{ route('enfermedads.show', $enfermedad->id) }}" class="text-gray-600 hover:text-blue-500">
                                            ...
                                            <a href="{{ route('enfermedads.edit', $enfermedad->id) }}" class="text-gray-600 hover:text-blue-500">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            ...
                                            <form method="POST" action="{{ route('enfermedads.destroy', $enfermedad->id) }}" onsubmit="return confirm('¿Eliminar este registro?');">
                                                @csrf
                                                @method('DELETE')
